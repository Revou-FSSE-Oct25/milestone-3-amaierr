...
function AddProductPage() {
    ...
    useEffect(() => {
        async function fetchData(){
            const categoryRes = await axios.get<Category[]>('https://api.escuelajs.co/api/v1/categories')
            setCategories(await categoryRes.data)
            
            const productRes = await axios.get<Product>(`https://api.escuelajs.co/api/v1/products/${params.id}`)
            setProduct(await productRes.data)
        }
    ...
    const onSubmit = () => {
        axios.put(`https://api.escuelajs.co/api/v1/products/${params.id}`, {
